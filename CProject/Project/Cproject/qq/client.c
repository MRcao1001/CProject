...
void regist(int sockfd)
{
  ...
  {
      if(0 == strncmp(REGIST_OK,pdu->caMsg,pdu->uiMsgType))
      {
          printf("注册成功，获得ID为:%u\n",pdu->uiTo);
          g_iId = pdu->uiTo;
      }
...
void handleGetFriendRespond(PDU *pdu)
{
  ...
  for(;i<num;i++)
  {
      memcpy(&uiId,pdu->caMsg+i*sizeof(unsigned int),sizeof(unsigned int));
      i++;
      printf("\t好友ID:%d\n",uiId);
  }
