...
{
    int ret = -1 ;
    ret = listen(sockfd,20);//20为监听客户端的数量;
    if(-1 == ret)
    {
        perror("listen");
        exit(EXIT_FAILURE);
...
void handleClienGetFriendtRequest(int sockfd)
{
    uint uiMsgiLen = g_pUsrList->uiLen*sizeof(uint);
    PDU *pdu = mkPDU(uiMsgiLen);
...
void handleClienPirvateChatRequest(int sockfd,PDU *pdu)
{
...
void handleClienGroupChatRequest(int sockfd,PDU *pdu)
{
     if(NULL == pdu)
     {
         return;
    ...
    while(1)
    {
        ...
        switch(pdu->uiMsgType)
        {
            ...
            case ENUM_MSG_TYPE_GET_FRIEND_REQUEST:
                  handleClienGetFriendtRequest(sockfd);
                 break; 
            case ENUM_MSG_TYPE_PRIVATE_CHAT_REQUEST:
                  handleClienPirvateChatRequest(sockfd,pdu);
                  break;  
            case ENUM_MSG_TYPE_GROUP_CHAT_REQUEST:
                 handleClienGroupChatRequest(sockfd,pdu);
                 break;            
            default:
                 break;
        }
    ...
    clientSockfd = accept(sockfd,(struct sockaddr *)&clientAddr,&iLen);
      if(-1 == clientSockfd)
      {
       ...
       printf("client sockfd = %d, ip:%s,port;%d\n",
        ...
        pthread_create(&thr,NULL,handleClient,(void *)clientSockfd);  
   }
...
int main (void)
{
    ...
    int sockfd = mkSocket();//产生sockfd用于监听客户端的连接;
