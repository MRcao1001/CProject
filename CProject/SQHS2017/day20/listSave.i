...
perNode *makeNode()
{
    perNode *newNode = (perNode*)malloc(sizeof(struct perNode));
...
# 27 "listSave.c"
           == newNode)
        newNode = (perNode *)malloc(sizeof(struct perNode));
...
# 50 "listSave.c"
              != p)
    {
        ret = fwrite(&p->data, sizeof(struct perInfo), 1, fp);
        if(ret < 1)
        {
            printf("write error!\n");
            break;
...
# 74 "listSave.c"
    ...
    while(fread(&temp->data, sizeof(struct perInfo), 1, fp) > 0)
    {
...
perInfo getData()
{
    system("clear");
    perInfo temp;
    memset(&temp, '\0', sizeof(struct perInfo));
    printf("姓名: ");
    scanf("%s", temp.name);
    getchar();
